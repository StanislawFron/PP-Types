...
void PrintGroups(string[] t, int perLine)
{
    ...
    {
        string postfix = (i == (t.Length - 1) ? "." : ", ");
        string completedText = t[i] + postfix;
        Console.Write((i + 1) % perLine == 0 ? completedText + "\n" : completedText);
    }
}
...
void PrintColumns(string[] t, int perLine, int width)
{
    for (int i = 0; i < t.Length; i++)
    {
        string text = t[i].Length > width ? t[i].Substring(0, width) : t[i].PadRight(width);
        Console.Write((i + 1) % perLine == 0 ? text + '\n' : text + "| ");
    }
}
